...
# 엑스트라틀 : 무작위로 (허술한/계산식없이) 노드분할 기준 선정 학습 ,
# 그레이디언트 부스팅 : 부모노드(트리)가 예측하고 오차를 자식노드(트리)에게 넘겨 학습
#           - 자식노드가 많아질 수록 오차는 줄어든다. ( 과대적합 주의 )

# 예시] Tree(1노드) 에서 실제 정답이 10을 목표로 하여 예측한 결과가 7이면 오차는 3 발생
#      Tree(2노드) 에서 이제 정답이 10을 목표로 하여 7 예측한다면 오차에서 1감소한 2추가하여 8예측하면 오차는 2발생
...
#   - 분할기준 : 자식노드를 만들때 256개 구간 기준으로 분할한다. < 빠르다 >
...
# 앙상블( 앞전 계산에 사용된 오차/결과를 다음/전체에 정확도 향상하는데 상쇄 방법 )
...
# 4. 히스토그램 기반 그레이디언트부스팅 : 연속된 샘플들을 구간(256개) 만들어서 모델 학습 , 전처리 시간이 부족하거나 학습 속도 개선 사용
...
cat = CatBoostClassifier() # 모델 객체 생성
scores = cross_validate( cat , train_input , train_target , n_jobs=-1)
print( np.mean( scores['test_score'] ) )  # 0.880540830832412
