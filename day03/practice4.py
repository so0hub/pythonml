...
iris_length = target_iris['SepalLengthCm']
iris_width = target_iris['SepalWidthCm']
...
print( lr.score( train_input , train_target ) ) # 훈련 세트 결정 계수 # 0.5395298831723283
print( lr.score( test_input , test_target ) ) # 테스트 세트 결정 계수 # 0.5423687498422736
# .score() 함수는 기본적으로 '결정계수($R^2$)'를 계산해서 반환하도록 만들어져 있습니다.
# 결정계수($R^2$)란?모델이 데이터의 실제 값을 얼마나 잘 예측하는지 나타내는 지표입니다. 
# 수치는 $0$에서 $1$ 사이로 나오며, $1$에 가까울수록 모델이 데이터를 완벽하게 예측했다는 뜻입니다.

# 3) 모델의 기울기(coef_)와 절편(intercept_)을 각각 출력하세요.
print( lr.coef_ ) # 기울기값 반환 # [0.76182638]
...
print( train_poly ) # [[21.16  4.6 ] ...

test_poly = np.column_stack( ( test_input**2 , test_input ) )
print( test_poly ) # [[18.49  4.3 ] ...
...
# 학습된 다항 회귀에서의 훈련 세트가 학습된 단순 선형 회귀에서의 훈련 세트보다 결정계수가 높다. 따라서 다항 회귀 모델이 데이터의 실제값을 더 완벽하게 예측하였다.
# 학습된 단순 선형 회귀에서의 테스트 세트가 학습된 다항 회귀에서의 테스트 세트보다 결정계수가 높다. 따라서 단순 선형 회귀 모델이 데이터의 실제값을 더 완벽하게 예측하였다.
