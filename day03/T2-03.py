...
# [3] 특성 공학 , 다항 특성 제공
# 다양한 특성을 추가로 만들어서 모델이 다양한 구조로 이해하기 위한 방법    # 과적합 발생
...
# 예제2] 1(기본값 제외) 2 , 3 , 2*2 , 3*3
poly = PolynomialFeatures( include_bias=False ) # 기본값 편향 없음( 기본 1 제외 )
poly.fit( [[2,3]] )
...
# 과대적합된 자료들을 스케일링  # 서로 다른 특성들 간에 단위가 다르므로 동일한 단위 사용하기 위해 ( 0 ~ 1 )
...
# 알파( alpha = 규제단위 ) , alpha 단위가 크면 클수록 가중치(기울기) 0으로 가깝다 만든다.
# 예] 길이 50 -> 10 줄였을 때 성능이 줄면  중요한 계수이다.
# 예] 너비 50 -> 10 줄였을 때 성능이 차이가 없으면 중요한 계수가 아니다.


from sklearn.linear_model import Ridge
ridge = Ridge() # 릿지 객체 생성
ridge.fit( train_scaled , train_target )    # 스케일링된 과대적합 학습
...
for alpth in alpha_list :
    ...
    # 최적 : 학습평가 와 테스트평가 간의 격차가 적은 것 : 알파10 가장 적합한 하이퍼파라미터 이다.
